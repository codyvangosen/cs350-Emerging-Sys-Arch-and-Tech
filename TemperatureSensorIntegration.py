...
#    2          Implemented lock object to mitigate glitches
#               associated with rapid button presses/multi-threading.
#--------------------------------------------------------------------
...
## at the same time.
## Lock to prevent multiple threads from writing to LCD at the same time
##
...
class TempMachine(StateMachine):
    ...
    ## Enables state change between Celsius and Fahrenheit modes when the button is pressed
    cycle = (
    ...
    def processButton(self):
        print('*** processButton')
        self.send("cycle")
        sleep(0.1)  # Give the state time to update

        # Immediately show updated display instead of waiting for the loop
        ...
        with self.displayLock:
            ...
            ## Updates the 16x2 LCD screen with both lines (timestamp on top, sensor data on bottom)
    ...
    ##
    ## Lock to prevent simultaneous access to the LCD display from multiple threads
    ##
    displayLock = Lock()
    ...
    def displayTemp(self):
        ...
        while not self.endDisplay:
            ...
            ## Setup line 1
            line1 = datetime.now().strftime('%b %d  %H:%M:%S\n')    ## This displays the current date and time in the format "Sep 27  08:38:13"

            ## Setup line 2
            if self.activeScale == 'C':

                ## Displays temperature in Celsius and relative humidity with percent symbol on one line
                line2 = f"T:{self.getCelsius():0.1f}C H:{self.getRH():0.1f}%"

            else:
                ## Displays temperature in Fahrenheit and relative humidity with percent symbol on one line
                line2 = f"T:{self.getFahrenheit():0.1f}F H:{self.getRH():0.1f}%"
...
## This button toggles the temperature display scale between Celsius and Fahrenheit
