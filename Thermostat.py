...
#    2          Added additional functionality for all three buttons.
#               Provided detailed comments explaining functionality.
#               LCD updates accordingly to state transitions.
#--------------------------------------------------------------------
...
class TemperatureMachine(StateMachine):
    ...
    def on_enter_heat(self):
        # This method is automatically called when entering the 'heat' state.
        # It ensures the LED state is set appropriately based on current temperature.
        self.updateLights()  # Reevaluate light behavior based on new state
    ...
    def on_exit_heat(self):
        # Called when exiting the 'heat' state.
        # Ensures the red LED is turned off to prevent incorrect indication.
        redLight.off()
    ...
    def on_enter_cool(self):
        # This method is automatically called when entering the 'cool' state.
        # It ensures the LED state is set appropriately based on current temperature.
        self.updateLights()  # Reevaluate light behavior based on new state
    ...
    def on_exit_cool(self):
        # Called when exiting the 'cool' state.
        # Ensures the blue LED is turned off to prevent incorrect indication.
        blueLight.off()
    ...
    def on_enter_off(self):
        # This method is called when entering the 'off' state.
        # Both LEDs should be turned off regardless of temperature.
        redLight.off()
        blueLight.off()
    ...
    ##Called when the state change button is pressed (green button).
    ##Triggers the state machine to move to the next state in the cycle.
    def processTempStateButton(self):
        if(DEBUG):
            print("Cycling Temperature State")

        self.send("cycle")  # Advance the state machine to the next state
    ...
    ## Called when the increase temperature button is pressed (red button).
    ## Increases the set point by 1 degree Fahrenheit.
    def processTempIncButton(self):
        if(DEBUG):
            print("Increasing Set Point")

        self.setPoint += 1
        self.updateLights()  # Update LED behavior based on new setpoint
    ...
    ## Called when the decrease temperature button is pressed (blue button).
    ## Decreases the set point by 1 degree Fahrenheit.
    ...
    ## This method checks the current state and adjusts the LED lights accordingly.
    ## The red LED indicates heating; blue LED indicates cooling.
    def updateLights(self):
        ## Make sure we are comparing temperatires in the correct scale
        temp = floor(self.getFahrenheit()) # Read and round down the current temperature
        redLight.off()
        blueLight.off() # Start from a known state by turning both LEDs off
        ...
        if self.current_state.id == "heat":
            ...
            if temp < self.setPoint:
                redLight.pulse()  # Below setpoint: fading red LED (heater is on)
            else:
                redLight.on()     # At or above setpoint: solid red LED (heater idle)

        elif self.current_state.id == "cool":
            # In cool mode:
            if temp > self.setPoint:
                blueLight.pulse()  # Above setpoint: fading blue LED (AC is on)
            else:
                blueLight.on()     # At or below setpoint: solid blue LED (AC idle)
    ...
    def setupSerialOutput(self):
        # Prepares a comma-delimited string with current state, temperature, and set point.
        # This string is sent over UART to the TemperatureServer every 30 seconds.
    ...
    ## This method continuously updates the 16x2 LCD with system status.
    ## It alternates the second line every 5 seconds between current temp and setpoint info.
    def manageMyDisplay(self):
        counter = 1 # Used to track when to send UART messages (every 30 sec)
        altCounter = 1 # Used to alternate second line of LCD every 5 seconds
        while not self.endDisplay:
            ...
            lcd_line_1 = current_time.strftime("%m-%d %H:%M\n")  # Line 1 shows date and time

            ## Setup Display Line 2
            if(altCounter < 6):
                # First 5 seconds: display current temperature in Fahrenheit
                lcd_line_2 = f"Temp: {floor(self.getFahrenheit())}\u00b0F     "
                altCounter += 1

            else:
                # Next 5 seconds: display state and set point
                lcd_line_2 = f"State:{self.current_state.id} {self.setPoint}\u00b0F"
    
                altCounter = altCounter + 1
                if(altCounter >= 11):
                    # Run the routine to update the lights every 10 seconds
                    # to keep operations smooth
                    self.updateLights() # Periodically update LED status every 10 seconds
                    altCounter = 1
            ...
            if((counter % 30) == 0):
                ...
                # Every 30 seconds, send status update to the TemperatureServer
                ser.write((self.setupSerialOutput() + "\n").encode('utf-8'))

                counter = 1
            else:
                counter = counter + 1
            sleep(1) # Delay loop to run every second

        ## Cleanup display
        screen.cleanupDisplay() # Clear the display and free up GPIO when shutting down
...
##
greenButton = Button(24) # State toggle button connected to GPIO 24
greenButton.when_pressed = tsm.processTempStateButton  # Bind to state cycle function

##
## Configure our Red button to use GPIO 25 and to execute
## the function to increase the setpoint by a degree.
##
redButton = Button(25) # Setpoint increase button connected to GPIO 25
redButton.when_pressed = tsm.processTempIncButton  # Bind to increment function

##
## Configure our Blue button to use GPIO 12 and to execute
## the function to decrease the setpoint by a degree.
##
blueButton = Button(12) # Setpoint decrease button connected to GPIO 12
blueButton.when_pressed = tsm.processTempDecButton  # Bind to decrement function
...
while repeat:
    try:
        ## wait
        sleep(30) # Sleep to avoid unnecessary CPU usage; work handled in threads
        ...
        tsm.endDisplay = True # Signal display thread to shut down
        sleep(1) # Short delay to allow threads to clean up